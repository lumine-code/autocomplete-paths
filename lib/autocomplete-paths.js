...
function activate() {
  ...
  // The paths are the project's file index now, so nothing here watches
  // `core.ignoredNames` or the rest of that policy: the index re-crawls itself
  // when any of it changes. `ignoredPatterns` is read per request, so it needs
  // no invalidation either.
